...
class ContasPagarModel extends AppTable {
	...
	{
		$res = $this->db
			->query('SELECT * FROM contas_pagar_descontos WHERE conta_pagar_id = ' . $this->id);
	...
	protected function beforeSave($data = null)
	{
		if (! $this->isNew()) {
			$this->query(
				sprintf('DELETE FROM contas_pagar WHERE parent_id = %s', $this->id)
			);
		}
	}
	...
	{
		$this->query(
			sprintf('DELETE FROM contas_pagar WHERE parent_id = %s', $id)
		);
	}
	...
	protected function afterSave($data = null, $entity = null)
	{
		if (! $this->isNew()) {
			$this->query(
				sprintf('DELETE FROM contas_pagar_descontos WHERE conta_pagar_id = %s', $entity->id)
			);
		}
			...
			foreach ($data['descontos'] as $desconto) {
				...
				$this->query(
					sprintf(
						"
						INSERT INTO 
							contas_pagar_descontos (conta_pagar_id, descricao, valor)
						VALUES(%s, '%s', %s)
						",
						$entity->id,
						$desconto['descricao'],
		...
		if ($this->data['recorrente'] == true) {
			...
			foreach ($periodo as $dt) {
				if ($i++ > 1) {
					...
					$newLancamento->setField('fornecedor_id', $this->fornecedor_id);
